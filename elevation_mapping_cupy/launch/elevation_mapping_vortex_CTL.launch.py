...
def generate_launch_description():
    ...
    core_param_path = os.path.join(share_dir, 'config', 'setups', 'vortex_CTL', 'parameters.yaml')
    sensor_param_path = os.path.join(share_dir, 'config', 'setups', 'vortex_CTL', 'sensor_GET.yaml')
    ...
    if not os.path.exists(sensor_param_path):
        raise FileNotFoundError(f"Sensor param file not found: {sensor_param_path}")

    # Declare launch arguments
    use_sim_time_arg = DeclareLaunchArgument(
        'use_sim_time',
        default_value='true',
        description='Use simulation (Gazebo/Vortex) clock if true'
    )
    ...
    rviz_config_arg = DeclareLaunchArgument(
        ...
        default_value=PathJoinSubstitution([
            share_dir, 'rviz', 'vortex_CTL.rviz'
        ]),
