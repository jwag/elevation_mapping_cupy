...
class ElevationMap:
    ...
    def move_to(self, position, R):
        ...
        # Ensure position is a 3x1 array
        position = xp.asarray(position).reshape(3, 1)
    ...
    def perform_soil_erosion(self,
        ...
        Args:
            ...
            intersected_inds (np.ndarray)(n,2): Indicies of the cells that are intersected by the GET blade during the sweep
            deposit_inds (np.ndarray)(n,2): Indicies of the cells where soil is deposited during the sweep
    ...
    def input_pointcloud(
        ...
        Sigma_b_r_MB: cp._core.core.ndarray = None,
        Sigma_Theta_MB: cp._core.core.ndarray = None,
    ):
        ...
        # If uncertainty is not provided, set it to zero
        if Sigma_b_r_MB is None:
            Sigma_b_r_MB = cp.zeros((3, 3), dtype=self.data_type)
        if Sigma_Theta_MB is None:
            Sigma_Theta_MB = cp.zeros((3, 3), dtype=self.data_type)
    ...
    def initialize_map(self, init_data, init_method="points", new_variance=None, points_interp_method="cubic", dilation_size_initialize=2):
        """Initializes the map according to some points and using an approximation according to method.

        Args:
            init_data (numpy.ndarray):   Points or heightmap used to initialize the map
            init_method (str): Method to initialize the map ['points', 'heightmap']
        ...
        """
        map_initializer = MapInitializer(self.initial_variance, xp=cp, method=init_method)
        self.clear()
        if init_method == "points":
            ...
            with self.map_lock:
                ...
                map_initializer(self.elevation_map, points, new_variance, points_interp_method)
        ...
        elif init_method == "heightmap":
            with self.map_lock:
                init_heightmap = cp.asarray(init_data, dtype=self.data_type)
                map_initializer(self.elevation_map, init_heightmap, new_variance)
...
if __name__ == "__main__":
    ...
    param = Parameter(
        use_chainer=False,
        weight_file=os.path.join(core_dir, "weights.dat"),
        plugin_config_file=os.path.join(core_dir, "plugin_config.yaml"),
    )
